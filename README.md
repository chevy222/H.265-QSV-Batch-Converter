...
- **`-noautorotate` 只解决一半，必须配 `-display_rotation 0`**：ffmpeg 5.1+ 会按旋转元数据自动插 `transpose`（软解路径会生效、QSV 硬解路径当前不生效），脚本显式关掉以免双重旋转；但 ffmpeg **仍会把输入流的 display matrix 复制到输出流**——画面已被手工转正、元数据却还在，播放器会再转一次（合计 180°）。三处 encoder 都在**两个** `-i` 之前加了 `-display_rotation 0`（ffmpeg ≥ 6 的输入选项）清掉它。实测：源 `rotation=90` → 产物无 rotation 侧数据；源无该标签则不受影响（对照）。
