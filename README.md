...
### mode 2 / 3 的 scale 尺寸必须取反（`WSC:HSC` → `HSC:WSC`）

`SC` 公式本身对横竖屏都正确，但**两条滤镜链消费 `WSC`/`HSC` 的顺序是相反的**：

| 链路 | 执行顺序 | 应传的尺寸 |
|---|---|---|
| `vpp_qsv`（mode 1） | 先 scale，后 transpose | `WSC:HSC`（旋转**前**） |
| CPU（mode 2/3） | 先 transpose，后 scale | `HSC:WSC`（旋转**后**） |

早期版本让两条链共用同一组 `WSC:HSC`，于是 CPU 链在 transpose（480×640 → 640×480）之后又被 scale 压回 `480×640`，画质被毁。实测对比：

| 源 | mode 1 | mode 2/3（共用 `WSC:HSC`） | mode 2/3（取反后） |
|---|---|---|---|
| 480×640 竖屏 | **640×480** ✓ | **480×640**（纵向拉伸 1.78×）✗ | 640×480 ✓ |
| 2560×1440 横屏 | **1080×1920** ✓ | **1920×1080**（方向完全错）✗ | 1080×1920 ✓ |

因为 mode 1 下封面走的也是有问题的 CPU 滤镜串，产物里封面方向会和主视频相反；同一处修复后两者一致。修好后 CPU 链的抽帧与 mode 1 **逐字节相同**。
...
> 只影响旋转模式（`ROT=0`/`1`）下的 mode 2/3。`ROT=2` 两条链都无 transpose，共用 `WSC:HSC` 是对的。
