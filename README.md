...
- **封面跟随旋转**：转码含旋转时，封面与画面同步旋转/缩放，**方向与尺寸与主视频一致**（重编码为高质量 mjpeg）；不旋转时原样复制
- **旋转元数据清零**：源带 `rotate` 显示矩阵时显式清掉，画面被手工转正后播放器不会再多转一次
- **保留全部音轨**：多音轨源不再只留第一条
...
> NVIDIA / AMD 独显**只能跑 mode 3**：mode 2 用的是 `-hwaccel qsv` + `hevc_qsv`，两者都是 Intel QSV 专属，纯 N 卡 / A 卡机器上连设备都建不出来。想全 GPU 需自行换成 `hevc_nvenc` / `hevc_amf`。
...
| 帧率   | 保持源帧率，不重采样                                                           |
| 音频增益 | `volumedetect` 扫描峰值 → 最大无削顶增益（上限 `MAXGAIN` dB）→ `volume` 滤镜。峰值取自第一条音轨，增益作用于全部音轨 |
| 音频编码 | 需要增益时转 AAC，码率取源音频码率并钳制在 64–192k；无需增益时 `-c:a copy`。映射用 `-map 0:a?`，**所有音轨都保留** |
...
| 4K 横屏 3840×2160 | 1080×1920 ✓ | 1080×1920 ✓ ⚠️ |

> ⚠️ 最后一行**不能**用来证明修复有效：新旧表达式算出来都是 0.5 倍、结果完全相同。真正体现差异的只有**竖屏那一行**（旧式子会把它压到只剩 31% 像素）。
...
不旋转模式（`ROT=2`）走的是同一条表达式，毛病也一样，只是少了转置：旧表达式对**竖屏源**会按「塞进 1920×1080 横屏框」去算，1080×1920 被压成 606×1080（同样只剩 31% 像素）；现在方向无关，保持 1080×1920。反过来，本身就不超上限的横屏源（如 1280×720）新旧都是原样保留——旧表达式的问题**只对竖屏源显现**，别拿横屏例子来说明。

### mode 2 / 3 的 scale 尺寸**不要**取反——`WSC:HSC` 两条链都用

`WSC`/`HSC` 不是算好的常数，而是**含 `iw`/`ih` 的表达式**，会在各自滤镜链的 scale 步骤里、按"该步骤当前拿到的帧"重新求值：

| 链路 | 执行顺序 | scale 看到的帧 | 正确的 w:h |
|---|---|---|---|
| `vpp_qsv`（mode 1） | 先 scale，后 transpose | 旋转**前** | `WSC:HSC` |
| CPU（mode 2/3） | 先 transpose，后 scale | 旋转**后** | `WSC:HSC`（同一顺序） |

正因为表达式会自适应，**两条链要的都是 `w=WSC` / `h=HSC`**。曾经有一次"优化"（提交 `305c028`）以为 CPU 链需要取反成 `HSC:WSC`——它并没有改变旋转，只是把目标尺寸整体颠倒，反而把已转好的帧压回旋转前的形状：

| 源 | mode 1 | mode 2/3（错误取反成 `HSC:WSC`） | mode 2/3（`WSC:HSC`，已恢复） |
|---|---|---|---|
| 480×640 竖屏 | **640×480** ✓ | **480×640**（拉伸、方向错）✗ | 640×480 ✓ |
| 2560×1440 横屏 | **1080×1920** ✓ | **1920×1080**（方向完全错）✗ | 1080×1920 ✓ |

恢复后只需一行（`ROTCPU` 在 `ROT=2` 时为空，一行覆盖所有情况）：

```bat
set "VFCPU=%ROTCPU%scale='%WSC%':'%HSC%'"
```

> 封面走的也是这条 CPU 滤镜串，所以修复后 mode 1 下封面与主视频方向一致。实测（480×640 竖屏测试片）恢复后 mode 1 与 mode 2 的产物 **md5 完全相同**，解码像素逐帧一致。
...
- **`-noautorotate` 只解决一半，必须配 `-display_rotation 0`**：ffmpeg 默认开启 autorotate（`ffmpeg -h full` 里 `-autorotate` 的说明只有 "automatically insert correct rotate filters"，没有版本门槛），会按旋转元数据自动插 `transpose`（软解路径会生效、QSV 硬解路径当前不生效），脚本显式关掉以免双重旋转；但 ffmpeg **仍会把输入流的 display matrix 复制到输出流**——画面已被手工转正、元数据却还在，播放器会再转一次（合计 180°）。三处 encoder 都在**两个** `-i` 之前加了 `-display_rotation 0`（ffmpeg ≥ 6 的输入选项）清掉它。实测：源 `rotation=90` → 产物无 rotation 侧数据；源无该标签则不受影响（对照）。
- **`OUTDIR` 的尾斜杠会让「源目录 == 输出目录」守卫失效**：`%%~f` 会保留尾斜杠（`for %I in ("C:\x\Desktop\") do @echo [%~fI]` → `[C:\x\Desktop\]`），永远不等于 `%CD%` 的 `C:\x\Desktop`，守卫此前一直在空转——从桌面运行时所有文件会被静默 SKIP。现在先 `md` 建目录、再用 `pushd` 归一化路径后比较（`pushd` 会去掉尾分隔符、解析短名）。
- **`set /p` 的两种命运**：stdin 是 NUL / 已 EOF 时它**立即返回**、变量保持未定义（脚本第 136 行的 `if not defined ROT set "ROT=0"` 正是靠这个兜底，所以 `convert_h265.bat < NUL` 能正常跑）；但如果 stdin 是一个**打开着却一直不给数据**的管道，或者没有控制台输入的计划任务，它会一直等下去。这类场景请传参数（`convert_h265.bat 1`）或设 `ASK=0`。
...
- **多音轨的增益取自第一条音轨**：`volumedetect` 只测 `a:0`，但增益会应用到全部音轨（脚本不再丢弃后续音轨）。代价是：若某条后续音轨的编码 MP4 装不下（如 DTS / PCM），`-c:a copy` 会在封装阶段直接失败，该文件明确报 `[FAIL]`——是可恢复的显式失败，好过静默丢轨。
- **临时目录用 `mkdir` 抢槽位保证唯一**：每次运行在 `%TEMP%` 下抢一个 `h265_<N>` 目录（`mkdir` 遇到已存在的目录会失败，这就是锁），4 个探测文件都放在里面，多开实例各用各的目录。
  > **不要用 `%RANDOM%` 做唯一命名**：cmd 随机数的种子精度是「秒」。本机实测同一秒启动的 4 个 cmd，`%RANDOM%` 打出的序列**完全相同**（都是 `12264 15960 26751 22810`），两个实例会拿到同一组文件名；而清理时是无条件删除，于是 A 实例会把 B 实例正在读的探测文件删掉。
- 探测文件**只在整批跑完、进入 `:summary` 时清理一次**（不是每个文件/每个目录清一次），删除顺序是先 `del` 掉 4 个文件、再 `rd` 掉目录（不加 `/s`，目录非空就删不掉，顺带当成"清理确实成功"的校验）。中途 Ctrl+C、直接关窗口、任务被杀都走不到 `:summary`，会残留一个空的/半空的 `h265_<N>` 目录——无害，下次运行抢下一个编号即可。
